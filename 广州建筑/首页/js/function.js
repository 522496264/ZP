...
// ajax({
//     url:'login.php',//地址 必传
//     type:'get', //get  post  默认类型  可选
//     date:{a:1,}  //数据
//     asynch:true/false;   //默认同步/异步
...
function ajax(option){
    ...
    var datebase=option.datebase==undefined?"text":option.datebase;
    //var success=function(d){};
    var xhr=new XMLHttpRequest();
    var date="";
    if(typeof option.date=="string"){
        date=option.date;
    }else if(typeof option.date=="object"){
        for(var i in option.date){
            date+=i+"="+option.date[i]+"&";
        }
        date=date.slice(0,-1);
    }
    ...
    if(type=="get"){
        if(date=""){
            xhr.open(type,option.url,asynch);
        }else {
            xhr.open(type,option.url+"?"+date,asynch);
        }
    ...
    xhr.onreadystatechange=function(){
        if(xhr.readyState==4){
            if(xhr.status==200){
                ...
                }else if(datebase=="xml"){
                    option.success&&option.success(xhr.responseXML);
                }else if(datebase=="json"){
                    option.success&&option.success(eval("("+xhr.response+")"));
                }
